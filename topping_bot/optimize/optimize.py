...
from topping_bot.optimize.toppings import Resonance, Substats, Topping, ToppingSet, Type
...
class Optimizer:
    ...
    def solve(self, reqs: Requirements):
        ...
        # if tart exists, populate dummy tart topping with stats
        if self.reqs.tart:
            tart_topping.flavor =  self.reqs.tart.substat
            # add a dummy line for main topping stat (workaround for Topping.str())
            tart_topping.substats = [(self.reqs.tart.substat, Decimal(0)), (self.reqs.tart.substat,  self.reqs.tart.target)] 
        
        yield from self.dfs([tart_topping], 0)
    ...
    def dfs(self, combo: List[Topping], idx):
        ...
        if (reason := self.prune(combo, self.toppings[idx:]))[0] != Prune.NONE:
            ...
            return reason
        if len(combo) == MAX_TOPPINGS_PER_SET:
    ...
    def prune(self, combo: List[Topping], toppings: List[Topping]):
        ...
        if self.solution and len(combo) != MAX_TOPPINGS_PER_SET:  # objective floor check
        ...
        if sum(overall_set_requirements.values()) > MAX_TOPPINGS_PER_SET - len(combo):  # combined topping req count check
            failures |= Prune.CONFLICTING_REQS_FAILURE

        if self.solution and len(combo) != MAX_TOPPINGS_PER_SET:
    ...
    def floor_case(self, combo: List[Topping], toppings: List[Topping], substats: Substats):
        ...
        n = MAX_TOPPINGS_PER_SET - len(combo)
        ...
        for match_count in range(n + 1):
            ...
            if len(potential_set) == MAX_TOPPINGS_PER_SET:
    ...
    def ceiling_case(self, combo: List[Topping], toppings: List[Topping], substats: Substats):
        ...
        potential_set = combo + nsmallest(MAX_TOPPINGS_PER_SET - len(combo), toppings, key=lambda x: x.value(substats))
        if len(potential_set) == MAX_TOPPINGS_PER_SET:
    ...
    def objective_case(self, combo: List[Topping], toppings: List[Topping]):
        for potential_req_count, potential_set in self.floor_case(combo, toppings, self.reqs.objective.types):
            unmatched_count = MAX_TOPPINGS_PER_SET - potential_req_count
            potential_value = sum(potential_set.raw(s) for s in self.reqs.objective.types)
            potential_value += self.reqs.best_possible_set_effect(combo, self.reqs.objective.types, MAX_TOPPINGS_PER_SET - unmatched_count)
    ...
    def combined_case(self, combo: List[Topping], toppings: List[Topping], substats: Substats, set_reqs: dict = None):
        ...
        if len(combo) != MAX_TOPPINGS_PER_SET:
            ...
            full_set = combo + self.floor_pool(MAX_TOPPINGS_PER_SET - len(combo), remaining_set, substats)
        else:
            full_set = combo

        if len(full_set) == MAX_TOPPINGS_PER_SET:
